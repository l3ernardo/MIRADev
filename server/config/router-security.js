...
security.post('/login',middleware.urlEncodedParser,middleware.passport.authenticate('ldapauth' , { failureRedirect: '/login',failureFlash: true}),function (req,res){
	if(req.user.hasAccess) {
		...
		//req.user.mail is not used since is an array for some users
		req.session.user = { uid : req.user.uid, cn:req.user.cn, groupName: req.user.groupName, mail:req.user.preferredIdentity};
