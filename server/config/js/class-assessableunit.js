			...
			if (accessrules.rules.admin) {
				...
				if(req.session.quarter == doc[0].CurrentPeriod){
					...
					db.find(searchobj).then(function(resdata) {
						...
						doc[0].CountryList = util.getIOTChildren(doc[0].IMT,"IMT",req);
		if(req.session.quarter == doc[0].CurrentPeriod){
			if(typeof doc[0].IOT !== "undefined"){
				doc[0].IOT = util.resolveGeo(doc[0].IOT, "IOT",req);
			}
		...
		if (docid == "") {
			...
			db.get(pid).then(function(pdata){
				...
				switch (doc[0].DocSubType) {
					...
					doc[0].Country = req.body.Countryid;
