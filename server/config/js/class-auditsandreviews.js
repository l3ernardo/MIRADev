...
var calculateARTab = {
  ...
  categorizeCULogic: function(doc, auditInter, parentAsmts, parentAU, tmp, parentISDeliveryDocs, parentCRMDocs) {
    try {
      ...
      if (doc[0].MIRABusinessUnit == "GTS" || (doc[0].MIRABusinessUnit == "GTS Transformation" && doc[0].ParentDocSubType != "Subprocess")) {
    ...
    try {
      ...
      for(var j = 0; j < parentAsmts.length; j++) {
        ...
        if (auditInter.parentid == parentAsmts[j]._id || auditInter.parentid == parentAsmts[j].parentid) {
          ...
          tmp.PeriodRating = auditInter.rating;
          //CU Scores and Size for Internal Audit
